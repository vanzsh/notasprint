...
- Top bar reads left to right: wordmark · `Reference` button (opens the Library) with the current motorsport and circuit in dim mono · Undo/Redo icon pair · agent chip · presentation icon · `Export` button (opens the export dialog). "Reference" is always the word for a starting layout; "Design inspiration" is always the word for a character applied to the live circuit. Never blur the two.
...
- **Simulated vehicles**: an oriented rectangle sized from the discipline's vehicle (5.6 × 2.0 m single-seater, 2.1 × 0.7 m motorcycle), never smaller than ~7 × 3 px, `--fg` fill, `--bg` hairline, no numbers, no colour. Canvas telemetry says `8 BIKES` or `12 CARS`.
