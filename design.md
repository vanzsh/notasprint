...
- Border radius: rounded rectangle, still clearly rectangular. `--radius: 6px` on every control (buttons, fields, selects, menus, tooltips, chips); `8px` on grouped controls and the circuit switcher and on dialogs; `10px` on the two large dialogs (Library, Export). Panels, bars and section headers are square. Never pills or squircles. Turn markers are circles because they are geometry, not UI.
...
- Panel tells the workflow in five parts: **Circuit identity** (always visible, the switcher) · **DESIGN** · **SIMULATE** · **PROJECT** · **Activity** (pinned). Three major collapsibles, each a 36px header with a 15px Barlow 700 uppercase label, 0.12em tracking, in `--accent` (the one place red names a section rather than a signal — it marks the three workflow stages) and a right-aligned mono **readout** while collapsed (`DESIGN  Flow 63 · 1 overtaking`, `SIMULATE  Not run`, `PROJECT  Export · Present`). Inside them, nested subsections are quieter: 28px headers, 10px muted labels, a lighter hairline, no boxes — hierarchy comes from type and spacing, never cards in cards.
- DESIGN nests Scores · Selected turn / Design loop · Sectors · Overtaking · Speed trace · Inspiration · Design brief · Warnings (only when there are any). SIMULATE holds the whole simulation. PROJECT nests Snapshots, then Export · Present, then Reset on its own hairline-separated row beside a sentence saying what it does. "Snapshots" is the product word for saved versions — never "versions", "commits" or other source-control language.
- Signals roll up: a failing brief or geometry warning shows `⚠ n` in `--accent` on the DESIGN header as well as on its subsection, so a collapsed stack still says something needs attention; a stale simulation shows `STALE` on SIMULATE. Signals stay visible when open.
- Default state: identity, DESIGN, Scores and the turn / design-loop subsection open; everything else collapsed. A section (and its parent) opens itself when content arrives — a brief set by the agent, a simulation run, a saved snapshot — unless the designer has toggled it by hand. Selecting a turn always opens its subsection.
...
- **Circuit switcher** (top of the Circuit section): the identity *is* the control — one raised `--surface-2` block with a `--line` border and 8px radius holding the 16px series mark, the motorsport as an uppercase label, `Reference ⇄` right-aligned in dim, and the circuit name in 26px Barlow beneath. Hover: `--line-strong` border, a slightly lighter surface, a 1px lift, and the label and `Reference` brighten to `--fg`. Click opens the Reference Library. Below it, outside the control: the tagline in 12px sans muted, then location · Reference (or Generated concept · `seed N` in mono, with the refresh icon) in dim sans. Then the hero metrics.
- **Project section**: Snapshots (save field + `Save snapshot`, the list, compare / restore), then `Export` · `Present` as two equal buttons, then Reset as a ghost button on its own row beside a dim sentence — apart from the productive actions. Reset is disabled while the circuit equals its starting layout and always confirms in a dialog (`Reset circuit?` · Cancel · `Reset circuit` in the `destructive` variant: `--accent` border and text, `--accent-dim` on hover — the one place a red control is allowed). The reset itself is an undoable commit.
...
- **Export dialog**: preview left (the actual SVG inside a hairline; the backdrop lightens when transparent), style list right as four bordered rows (name + one-line blurb; the chosen row has a `--fg` border on `--surface-2`), a `Transparent background` switch (`ui/switch`, 28 × 16, disabled for JSON), then one primary split button: `Export PNG` with a `▾` that picks PNG · SVG · JSON and renames the button. Drawings are 2400px wide. Styles: technical (grid, turn numbers and names, sectors, braking chevrons, scale bar), presentation (name, location, four hero metrics), minimal (asphalt only), analysis (presentation plus scores, sectors and strong braking zones). Every drawing carries "NotASprint · design analysis, not survey data" in dim 10px.
