...
import { CIRCUITS, REFERENCES } from "./circuits";
...
import { resolveSeries, SERIES, SERIES_IDS, seriesById } from "./series";
import { compareSimulations, type CompactSimResult, type SimResult } from "./simulation";
import { clearBrief, commit, getState, loadCircuit, loadCustom, restoreVersion, runSimulation, saveVersion, setBrief, setCompare, snapshot, undo } from "./store";
...
// The motorsport and the reference the live circuit came from, as the agent reads them on every response.
const motorsport = (c: Circuit) => {
  const s = seriesById(c.series);
  return { id: s.id, name: s.name, vehicle: s.noun, emphasis: s.emphasis, vehicle_model: { top_speed_kmh: Math.round(s.vehicle.vMax * 3.6), lateral_g: r1(s.vehicle.aLat / 9.81), braking_g: r1(s.vehicle.aBrk / 9.81), min_corner_radius_m: s.vehicle.minRadius }, typical_lap_length_m: [s.lengthBand[1], s.lengthBand[2]] };
};
const reference = (c: Circuit) =>
  c.custom !== undefined
    ? { kind: "custom" as const, seed: c.custom, note: "Generated by NotASprint for this discipline; create_custom_circuit with the same seed reproduces it." }
    : { kind: c.location ? ("reference" as const) : ("original" as const), id: c.id, name: c.name, location: c.location, character: c.character, example_of: c.inspiration ?? "mixed" };
const library = () =>
  Object.fromEntries(SERIES_IDS.map((s) => [s, REFERENCES[s].map((r) => ({ id: r.id, name: r.name, location: r.location, character: r.character, example_of: r.inspiration ?? "mixed" }))]));
const REFERENCE_NOTE = "Reference layouts are schematic interpretations drawn for design inspiration — the character and rough proportions of the place, not survey geometry or official data.";

function summary(c: Circuit, a: Analysis) {
  return {
    circuit: c.name, circuit_id: c.id, motorsport: motorsport(c), reference: reference(c),
    length_m: Math.round(a.length), est_lap: fmtLap(a.lapTime), track_width_m: c.trackWidth,
...
export const tools: Tool[] = [
  {
    ...
    execute: async () => {
      ...
      return ok({
        ...
        motorsports: SERIES_IDS.map((s) => ({ id: s, name: SERIES[s].name, emphasis: SERIES[s].emphasis })),
        reference_library: library(),
        reference_note: `${REFERENCE_NOTE} load_reference_circuit loads one; create_custom_circuit generates a fresh concept for a motorsport.`,
      });
    },
  },
  {
    ...
    execute: async () => {
      ...
      return ok({
        ...
        design_guidance: [
          `Design for the current motorsport (${seriesById(c.series).name}): ${seriesById(c.series).emphasis} Scores and warnings are already read against its vehicle model.`,
  ...
  },
  {
    ...
    inputSchema: {
      type: "object",
      properties: {
        motorsport: { type: "string", enum: [...SERIES_IDS], description: "f1, fe or motogp. Phrases like 'Formula E' or 'bikes' also resolve." },
        seed: { type: "integer", description: "Omit for a new concept; repeat a seed to get the same layout again." },
      },
      required: ["motorsport"],
    },
    annotations: { destructiveHint: true },
    execute: async ({ motorsport: m, seed }) =>
      run(() => {
        ...
        if (!s) throw new Error(`Unknown motorsport "${m}". Use one of: ${SERIES_IDS.join(", ")}.`);
        const c = loadCustom(s.id, seed === undefined || seed === null ? undefined : Number(seed), "agent");
        return afterWrite(`Generated ${c.name} · seed ${c.custom}`);
      }),
  },
  {
    name: "undo_changes",
  ...
  },
  {
    ...
    execute: async ({ format, style }) => {
      ...
      if (fmt === "json") { const body = exportJSON(circuit, analysis); if (typeof document !== "undefined") download(`${circuit.id}.json`, body, "application/json"); return body; }
      const st = (EXPORT_STYLES.includes(style as ExportStyle) ? style : "technical") as ExportStyle;
      try { const r = await exportDrawing(circuit, analysis, fmt, st); return ok({ ok: true, ...r }); } catch (e) { return ok({ ok: false, error: (e as Error).message }); }
    },
  },
];
